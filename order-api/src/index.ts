...
import { connectToMongoDB } from './utilities/mongodb_manager';
...
async function app() {
  ...
  server.use(
    ...
    swaggerUi.setup(swaggerDocument, {
      explorer: true,
      customSiteTitle: 'Order-Swagger',
    })
  );
  server.get('/order/swagger.json', (req: Request, res: Response) => {
    res.setHeader('Content-Type', 'application/json');
    res.send(swaggerDocument);
  });

  server.use(`/order`, routes);
  ...
  server.listen(port, () => {
    ...
    console.log(`Seving API DOC >> http://localhost:${port}/order/swagger`);
  });
}
app();
